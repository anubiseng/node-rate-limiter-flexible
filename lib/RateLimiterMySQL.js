...
class RateLimiterMySQL extends RateLimiterStoreAbstract {
  ...
  _getRateLimiterRes(rlKey, changedPoints, result) {
    ...
    res.remainingPoints = Math.max(row.Quota - res.consumedPoints, 0);
    ...
    return new Promise((resolve, reject) => {
      conn.query('BEGIN', (errBegin) => {
        ...
        conn.query(q, values, (errUpsert) => {
          ...
          conn.query('SELECT points, expire, Quota FROM ??.?? INNER JOIN ??.Users ON `Username`=`key` WHERE `key` = ?;', [this.dbName, this.tableName, this.dbName, key], (errSelect, res) => {
            if (errSelect) {
              conn.rollback();
  ...
  _get(rlKey) {
    ...
    return new Promise((resolve, reject) => {
      this._getConnection()
        .then((conn) => {
          conn.query(
            'SELECT points, expire, Quota FROM ??.?? INNER JOIN ??.Users ON `Username`=`key` WHERE `key` = ? AND (`expire` > ? OR `expire` IS NULL)',
            [this.dbName, this.tableName, this.dbName, rlKey, Date.now()],
            (err, res) => {
              if (err) {
                reject(err);
